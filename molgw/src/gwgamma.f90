...
!=====
 ...
 real(dp),allocatable     :: selfenergy_g0w0(:,:)
...
!       do mstate=nsemin,nsemax
!         kcmstate = index_prodstate(kcstate,mstate) + nprodbasis * (ispin-1)
!         bra(:,mstate) = wpol%residu_left(kcmstate,:)
!       end do
!     else
!     endif
             ...
             do iomegai=-nomegai,nomegai
               selfenergy_omega_gamma(iomegai,mstate,1,ispin) = selfenergy_omega_gamma(iomegai,mstate,1,ispin) &
                        - bra(mstate,kstate) * bra(istate,bstate) * vcoul                          &  
                          ...
                          *  REAL(  1.0_dp / ( energy_qp(istate,ispin) - energy_qp(bstate,ispin) - wpol%pole(stpole) + ieta )  , dp ) 
             enddo
           enddo
 ...
 allocate(selfenergy_g0w0(nsemin:nsemax,nspin))
 forall(astate=nsemin:nsemax)
   selfenergy_g0w0(astate,:) = selfenergy(astate,astate,:)
   selfenergy_omega(0,astate,1,:) = selfenergy_g0w0(astate,:) + selfenergy_omega_sox(0,astate,1,:) + selfenergy_omega_gamma(0,astate,1,:)
 end forall
   ...
   do astate=nsemin,nsemax
     ...
     do iomegai=-nomegai,nomegai
       write(selfenergyfile,'(20(f12.6,2x))') ( omegai(iomegai) + energy_qp(astate,:) )*Ha_eV,            &
                                          ( selfenergy_omega_sox(iomegai,astate,1,:) + selfenergy_omega_gamma(0,astate,1,:) )*Ha_eV,   &
