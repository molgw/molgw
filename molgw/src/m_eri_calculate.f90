...
!=========================================================================
subroutine calculate_eri_3center_sca(print_eri_,basis,auxil_basis)
 use m_tools,only: boys_function
 implicit none
 logical,intent(in)           :: print_eri_
 type(basis_set),intent(in)   :: basis
...
!SCALAPACK
 ...
 integer,external             :: NUMROC,INDXL2G,INDXG2P
!=====
! variables used to call C
 integer(C_INT)               :: am1,am2,am3,am4
 integer(C_INT)               :: ng1,ng2,ng3,ng4
...
!=====
 ...
 write(*,*) 'FBFB ERI 3CENTER with SCALAPACK grid',nprow_rd,' x ',npcol_rd

 meri2 = NUMROC(nauxil_2center ,block_row,iprow_rd,first_row,nprow_rd)
 neri2 = NUMROC(auxil_basis%nbf,block_col,ipcol_rd,first_col,npcol_rd)
 nauxil_3center = meri2

 call DESCINIT(desc_2center,nauxil_2center,auxil_basis%nbf,block_row,block_col,first_row,first_col,cntxt_rd,MAX(1,meri2),info)

 !
 ! First, copy the part of eri_2center_m1 that is actually needed and deallocate
 ! the rest
 allocate(eri_2tmp(meri2,neri2))
 do ibf_local=1,meri2
   ibf_global = INDXL2G(ibf_local,block_row,iprow_rd,first_row,nprow_rd)
   eri_2tmp(ibf_local,:) = eri_2center_m1(:,ibf_global)
 enddo
 ...
 meri3 = NUMROC(nauxil_2center,block_row,iprow_rd,first_row,nprow_rd)
 neri3 = NUMROC(npair         ,block_col,ipcol_rd,first_col,npcol_rd)

 call DESCINIT(desc_3center,nauxil_2center,npair,block_row,block_col,first_row,first_col,cntxt_rd,MAX(1,meri3),info)

 call clean_allocate('3-center integrals',eri_3center,meri3,neri3)


 !
 ! Allocate this temporary array
 ! 
 mtmp3 = NUMROC(auxil_basis%nbf,block_row,iprow_rd,first_row,nprow_rd)
 ntmp3 = NUMROC(npair          ,block_col,ipcol_rd,first_col,npcol_rd)

 call DESCINIT(desc_3tmp,auxil_basis%nbf,npair,block_row,block_col,first_row,first_col,cntxt_rd,MAX(1,mtmp3),info)

 call clean_allocate('3-center TMP integrals',eri_3tmp,mtmp3,ntmp3)
 ...
 do ishell=1,nshell_auxil
   ...
   skip_it = .TRUE.
   do ibf=1,ni
     ibf_global = ibf_start + ibf
     skip_it = skip_it .AND. ( iprow_rd == INDXG2P(ibf_global,block_row,0,first_row,nprow_rd) )
   enddo
     ...
     if(amk+aml>=ami) then
       ...
       do lbf=1,nl
         do kbf=1,nk
           ...
           do ibf=1,ni
             ibf_global = ibf_start + ibf
             ibf_local = rowindex_global_to_local('R',ibf_global)
             if( ibf_local /= 0 ) eri_3tmp(ibf_local,ipair) = integrals_cart(ibf,1,kbf,lbf)
           enddo
         enddo
       enddo
       ...
       do lbf=1,nl
         do kbf=1,nk
           ...
           do ibf=1,ni
             ibf_global = ibf_start + ibf
             ibf_local = rowindex_global_to_local('R',ibf_global)
             if( ibf_local /= 0 ) eri_3tmp(ibf_local,ipair) = integrals_cart(kbf,lbf,ibf,1)
           enddo
         enddo
       enddo
   ...
   ibf_start = ibf_start + ni

 enddo

 call PDGEMM('N','N',nauxil_2center,npair,auxil_basis%nbf,    &
              1.0_dp,eri_2tmp,1,1,desc_2center,               &
              eri_3tmp,1,1,desc_3tmp,                         &
              0.0_dp,eri_3center,1,1,desc_3center)


 call clean_deallocate('3-center TMP integrals',eri_3tmp)
