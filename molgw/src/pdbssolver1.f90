...
!     $     6, WORK( INDWORK ) )
!
!     Restore the negative eigenvalues by symmetry.
!
T_SYM = MPI_WTIME()
DO I = N, 1, -1
   LAMBDA( 2*I-1 ) = SQRT( LAMBDA( I ) )
   LAMBDA( 2*I ) = -LAMBDA( 2*I-1 )
END DO
!      IF ( MYROW+MYCOL .EQ. 0 ) THEN
!         DO I = 1, TWON
!            WRITE( *, * ) 'lambda(', I, ', 1) =', LAMBDA( I ), ';'
!         END DO
!      END IF
...
!
!     X(:, 2J-1:2J) =
!     [Psi(:, J)*lambda_+(J)+Phi(:, J), Psi(:, J)*lambda_+(J)-Phi(:, J);
!      Psi(:, J)*lambda_+(J)-Phi(:, J), Psi(:, J)*lambda_+(J)+Phi(:, J)]
!
!     Y(:, 2J-1:2J) =
!     [Phi(:, J)/lambda_+(J)+Psi(:, J), Phi(:, J)/lambda_+(J)-Psi(:, J);
!      Phi(:, J)/lambda_+(J)-Psi(:, J), Phi(:, J)/lambda_+(J)+Psi(:, J)]
!
!     where J = 1:N.
!
T_VEC2 = MPI_WTIME()
DO J = 0, N-1
   CALL PDCOPY( N, WORK( INDPHI ), 1, J+1, DESCPHI, 1, &
        X, IX, JX + 2*J, DESCX, 1 )
   CALL PDSCAL( N, HALF, X, IX, JX + 2*J, DESCX, 1 )
   CALL PDAXPY( N, HALF*LAMBDA( 2*J+1 ), WORK( INDPSI ), 1, J+1, &
        DESCPSI, 1, X, IX, JX + 2*J, DESCX, 1 )
   CALL PDCOPY( N, WORK( INDPHI ), 1, J+1, DESCPHI, 1, &
        X, IX + N, JX + 2*J, DESCX, 1 )
   CALL PDSCAL( N, -HALF, X, IX + N, JX + 2*J, DESCX, 1 )
   CALL PDAXPY( N, HALF*LAMBDA( 2*J+1 ), WORK( INDPSI ), 1, J+1, &
        DESCPSI, 1, X, IX + N, JX + 2*J, DESCX, 1 )
   CALL PDCOPY( N, X, IX + N, JX + 2*J, DESCX, 1, &
        X, IX, JX + 2*J+1, DESCX, 1 )
   CALL PDCOPY( N, X, IX, JX + 2*J, DESCX, 1, &
        X, IX + N, JX + 2*J+1, DESCX, 1 )
END DO
!
DO J = 0, N-1
   CALL PDCOPY( N, WORK( INDPSI ), 1, J+1, DESCPSI, 1, &
        Y, IY, JY + 2*J, DESCY, 1 )
   CALL PDSCAL( N, HALF, Y, IY, JY + 2*J, DESCY, 1 )
   CALL PDAXPY( N, HALF/LAMBDA( 2*J+1 ), WORK( INDPHI ), 1, J+1, &
        DESCPHI, 1, Y, IY, JY + 2*J, DESCY, 1 )
   CALL PDCOPY( N, WORK( INDPSI ), 1, J+1, DESCPSI, 1, &
        Y, IY + N, JY + 2*J, DESCY, 1 )
   CALL PDSCAL( N, -HALF, Y, IY + N, JY + 2*J, DESCY, 1 )
   CALL PDAXPY( N, HALF/LAMBDA( 2*J+1 ), WORK( INDPHI ), 1, J+1, &
        DESCPHI, 1, Y, IY + N, JY + 2*J, DESCY, 1 )
   CALL PDCOPY( N, Y, IY + N, JY + 2*J, DESCY, 1, &
        Y, IY, JY + 2*J+1, DESCY, 1 )
   CALL PDCOPY( N, Y, IY, JY + 2*J, DESCY, 1, &
        Y, IY + N, JY + 2*J+1, DESCY, 1 )
END DO
