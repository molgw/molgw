...
! force(:,:) = 0.0_dp
 ...
 ! Fock to be tested 
 ! 
! call issue_warning('force set to zero: before exchange')
! force(:,:) = 0.0_dp
     ...
     do lbf=1,basis%nbf
       do kbf=1,basis%nbf
         do jbf=1,basis%nbf
           do ibf=1,basis%nbf
             ! Hartree
             force(:,iatom) = force(:,iatom) + psp_matrix(ibf,jbf,ispin,iatom,:) * eri(ibf,jbf,kbf,lbf) * p_matrix(kbf,lbf,ispin)
             ! Exchange
!             force(:,iatom) = force(:,iatom) - 0.5_dp*psp_matrix(ibf,kbf,ispin,iatom,:) * eri(ibf,jbf,kbf,lbf) * p_matrix(jbf,lbf,ispin)
!             force(:,iatom) = force(:,iatom) - 0.5_dp*psp_matrix(jbf,lbf,ispin,iatom,:) * eri(ibf,jbf,kbf,lbf) * p_matrix(ibf,kbf,ispin)
           enddo
         enddo
       enddo
     enddo
...
! call issue_warning('zero forces here')
! force(:,:) = 0.0_dp
