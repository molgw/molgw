...
!     hamiltonian_nucleus_grad(ibf:ibf+ni-1,jbf:jbf+nj-1,1) = MATMUL( TRANSPOSE(cart_to_pure_norm(li)%matrix(:,:)) , &
!                                                              MATMUL( matrix_cart_gradAx(:,:) , cart_to_pure_norm(lj)%matrix(:,:) ) ) 
!     hamiltonian_nucleus_grad(ibf:ibf+ni-1,jbf:jbf+nj-1,2) = MATMUL( TRANSPOSE(cart_to_pure_norm(li)%matrix(:,:)) , &
!                                                              MATMUL( matrix_cart_gradAy(:,:) , cart_to_pure_norm(lj)%matrix(:,:) ) ) 
!     hamiltonian_nucleus_grad(ibf:ibf+ni-1,jbf:jbf+nj-1,3) = MATMUL( TRANSPOSE(cart_to_pure_norm(li)%matrix(:,:)) , &
!                                                              MATMUL( matrix_cart_gradAz(:,:) , cart_to_pure_norm(lj)%matrix(:,:) ) ) 
!     hamiltonian_nucleus_grad(ibf:ibf+ni-1,jbf:jbf+nj-1,4) = MATMUL( TRANSPOSE(cart_to_pure_norm(li)%matrix(:,:)) , &
!                                                              MATMUL( matrix_cart_gradBx(:,:) , cart_to_pure_norm(lj)%matrix(:,:) ) ) 
!     hamiltonian_nucleus_grad(ibf:ibf+ni-1,jbf:jbf+nj-1,5) = MATMUL( TRANSPOSE(cart_to_pure_norm(li)%matrix(:,:)) , &
!                                                              MATMUL( matrix_cart_gradBy(:,:) , cart_to_pure_norm(lj)%matrix(:,:) ) ) 
!     hamiltonian_nucleus_grad(ibf:ibf+ni-1,jbf:jbf+nj-1,6) = MATMUL( TRANSPOSE(cart_to_pure_norm(li)%matrix(:,:)) , &
!                                                              MATMUL( matrix_cart_gradBz(:,:) , cart_to_pure_norm(lj)%matrix(:,:) ) ) 
