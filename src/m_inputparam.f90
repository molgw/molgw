...
subroutine setup_nuclei(inputfile,basis,auxil_basis,small_basis,ecp_basis,ecp_auxil_basis,ecp_small_basis)
  ...
  !=====
  integer              :: natom_read
  integer              :: atom_number,iatom,ielement_ecp
  integer              :: info,info1,info2,xyzfile
  character(len=12)    :: atom_symbol
  ...
  if( LEN(TRIM(xyz_file)) == 0 ) then
    ...
    do iatom=1,natom+nghost
      ...
      write(stdout,'(a)') '__'//atom_symbol//'__'
      write(stdout,'(a)') LEN(TRIM(atom_symbol))
      read(atom_symbol,*,iostat=info) zatom_read(iatom)
      ! If it fails, then assumes it is a character
      if( info /=0 ) then
        zatom_read(iatom) = element_number(atom_symbol)
      endif
      write(stdout,'(a,es12.6)') '__Z',zatom_read(iatom)
    enddo
  else
    !
