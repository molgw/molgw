     ...
     if( calc_type%selfenergy_approx == GnW0 .AND. istep_gw > 1 ) then
       ...
       if(kappa_hybrid/=one) then ! Print the double-hybrid RPA correlation energy
         ...
         if( ALLOCATED(dft_xc) ) then
           ...
           allocate(dft_xc(2))
           dft_xc(:)%nspin = nspin
           dft_xc(1)%id = XC_LDA_C_PW
           dft_xc(2)%id = XC_LDA_C_PW_RPA
           ...
           call dft_exc_vxc_batch(BATCH_SIZE,basis,occupation,c_matrix,matrix_tmp,ErpaP)
           call clean_deallocate('XC operator RPA+',matrix_tmp)
           write(stdout,'(/,a,f19.10)') ' Scaled E(LHEG) - E(LRPA) correlation energy (Ha):',ErpaP
           ErpaP=en_mbpt%rpa+ErpaP
           write(stdout,'(/,a,f19.10)') ' Scaled RPA+ correlation energy (Ha):',ErpaP
         endif
       endif
       ...
       if( ABS(en_mbpt%rpa) > 1.e-6_dp) then
         ...
         if(kappa_hybrid/=one) then ! Print the double-hybrid RPA Total energy
           write(stdout,'(a,f19.10)') ' RPA+ Total energy (Ha): ',en_mbpt%total-en_mbpt%rpa+ErpaP
         endif
       endif
