...
!=========================================================================
subroutine update_hamiltonian(basis,hamiltonian_kinetic,hamiltonian_nucleus)
...
!=====
 ...
 if( has_auxil_basis ) then
   ...
   if( TRIM(capitalize(auxil_basis_name(1))) /= 'AUTO' .AND. TRIM(capitalize(auxil_basis_name(1))) /= 'PAUTO' ) then
     call init_basis_set(basis_path,auxil_basis_name,ecp_auxil_basis_name,gaussian_type,new_auxil_basis)
   else
     call init_auxil_basis_set_auto(auxil_basis_name,basis,gaussian_type,auto_auxil_fsam,auto_auxil_lmaxinc,new_auxil_basis)
   endif
 endif
 ...
 if(has_auxil_basis) then
   call calculate_eri_2center_scalapack(new_auxil_basis,0.0_dp)
   call calculate_eri_3center_scalapack(basis,new_auxil_basis,0.0_dp)
   call destroy_basis_set(new_auxil_basis)
 else
...
!    close(file_iter_norm)
   ...
   call propagate_orth(nstate,basis,time_step,c_matrix_orth_hist_cmplx(:,:,:,1),c_matrix_cmplx,h_small_hist_cmplx(:,:,:,n_hist+1),x_matrix,prop_type)
 ...
 case('PC5','PC6')
   ...
   do i_iter=1,n_iter
    ...
    !--5--PROPAGATION----| C(0)---U[H(1/2)]--->C(!1)
    call propagate_orth(nstate,basis,time_step,c_matrix_orth_cmplx,c_matrix_cmplx,h_small_hist_cmplx(:,:,:,n_hist:n_hist+1),x_matrix,"ETRS")
 ...
 case('PC7' )
   ...
   do i_iter=1,n_iter
     ...
     if(i_iter/=n_iter) then
       c_matrix_orth_hist_cmplx(:,:,:,1)=c_matrix_orth_cmplx(:,:,:)
       call propagate_orth(nstate,basis,time_step,c_matrix_orth_hist_cmplx(:,:,:,1),c_matrix_cmplx,h_small_hist_cmplx(:,:,:,1:2),x_matrix,"ETRS")
     end if
   end do
...
!=========================================================================
subroutine propagate_orth_ham_2(nstate,basis,time_step_cur,c_matrix_orth_cmplx,c_matrix_cmplx,h_small_hist2_cmplx,x_matrix,prop_type)
 implicit none
 integer,intent(in)          :: nstate
 type(basis_set),intent(in)  :: basis
 real(dp),intent(in)         :: time_step_cur
