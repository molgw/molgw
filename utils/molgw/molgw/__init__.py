...
try:
    ...
    sys.exit(
        'import yaml failed. Please install this module, for instance with  "pip install PyYAML".'
    )
try:
...
Ha_eV = 27.21138505
c_speedlight = (
    137.035999074  # speed of light in atomic units (fine-structure constant inverse)
)
periodic_table = [ 'H',                                                                                                  'He',
                   'Li', 'Be',                                                              'B',  'C',  'N',  'O',  'F', 'Ne',
...
def run(
    ...
    **kwargs,
    ...
    process = subprocess.Popen(
        cmd, cwd=tmp_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    with open(filename, "w") as f:
        ...
        for atom in atom_list:
            f.write(
                "{:2}   {:14.8f} {:14.8f} {:14.8f}\n".format(
                    atom[0],
                    float(atom[1]) * bohr_ang,
                    float(atom[2]) * bohr_ang,
                    float(atom[3]) * bohr_ang,
                )
            )
...
def get_homo_energy(approx, calc):
    ...
    energies.sort()
    return energies[
        int(calc["physical system"]["electrons"])
        - 1
        - 2 * (min(list(calc[key]["spin channel 1"].keys())) - 1)
    ]
...
def get_energy(approx, state, calc):
    ...
    energies_occ = [
        float(ei) for i, ei in calc[key]["spin channel 1"].items() if i <= homo_key
    ]
    energies_virt = [
        float(ei) for i, ei in calc[key]["spin channel 1"].items() if i > homo_key
    ]
...
def get_lumo_energy(approx, calc):
    ...
    if calc["input parameters"]["nspin"] == 1:
        ...
        energies += [float(ei) for ei in calc[key]["spin channel 2"].values()]
    energies.sort()
    index = int(calc["physical system"]["electrons"]) - 2 * (
        min(list(calc[key]["spin channel 1"].keys())) - 1
    )
...
def parse_yaml_files(directory):
    ...
    print(f"{len(yaml_files)} molgw.yaml files identified in directory {directory}")
...
def scs_bethe_au(v_au, nelec, ionization, charge=1.0):
    return (
        ...
        * charge**2
        / v_au**2
        * math.log(2.0 * v_au**2 / ionization)
    )
...
class Molgw_input:
    ...
    def __init__(self, origin):
        ...
        if isinstance(origin, dict):
            ...
            raise TypeError(
                "Molgw_input should be initialized with a dictionary or a file path"
            )
    ...
    def check(self):
        ...
        mandatory = [
            k for k in input_keywords.keys() if input_keywords[k]["mandatory"] == "yes"
        ]
        for k in mandatory:
            if k not in [key for key in pyinput_lower]:
...
class Molgw_output:
    ...
    def __init__(self, origin):
        if isinstance(origin, dict):
            ...
            raise TypeError(
                "Molgw_output should be initialized with a dictionary or a yaml file path"
            )
